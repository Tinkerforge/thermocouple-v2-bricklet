...
#define FID_GET_ERROR_STATE 7

#define FID_CALLBACK_TEMPERATURE 4
#define FID_CALLBACK_ERROR_STATE 8
...
typedef struct {
	...
	bool over_under;
	bool open_circuit;
} __attribute__((__packed__)) GetErrorState_Response;

typedef struct {
	TFPMessageHeader header;
	bool over_under;
	bool open_circuit;
} __attribute__((__packed__)) ErrorState_Callback;
